...
void ReadVectors() {
  FILE *fvin;
  FILE *fvout;
  long long word, words, a;
  char vector_filename[MAX_STRING];

  strcpy(vector_filename, vector_files);
  strcat(vector_filename, ".vin.bin");
  if(!(fvin = fopen(vector_filename, "rb"))) {
    printf("WARNING: %s is not found. Random initialization. \n", vector_filename);
    return;
  }
  strcpy(vector_filename, vector_files);
  strcat(vector_filename, ".vout.bin");
  if(!(fvout = fopen(vector_filename, "rb"))) {
    printf("WARNING: %s is not found. Random initialization. \n", vector_filename);
    return;
  }

  fscanf(fvin, "%lld", &words);
  fscanf(fvin, "%lld", &word);
  fscanf(fvout, "%lld", &words);
  fscanf(fvout, "%lld", &word);
  ...
  while(1) {
    ...
    for (a = 0; a < layer1_size; a++) fread(&syn1neg[a + word * layer1_size], sizeof(float), 1, fvout);
  }
  fclose(fvin);
  fclose(fvout);
...
void InitNet() {
  ...
  if (negative>0) {
    ...
    for (a = 0; a < vocab_size; a++) for (b = 0; b < layer1_size; b++) syn1neg[a * layer1_size + b] = 0;
  }
  for (a = 0; a < vocab_size; a++) for (b = 0; b < layer1_size; b++) {
...
void TrainModel() {
  ...
  FILE *fo;
  FILE *fvout;
  char vin_name[MAX_STRING];
  char vout_name[MAX_STRING];

  FILE *fvout_txt;
  char vout_name_txt[MAX_STRING];
  ...
  fo = fopen(vin_name, "wb");
  fvout = fopen(vout_name, "wb");
  fvout_txt = fopen(vout_name_txt, "wb"); 
  if (classes == 0) {
    // Save the word vectors
    fprintf(fo, "%lld %lld\n", vocab_size, layer1_size);
    fprintf(fvout, "%lld %lld\n", vocab_size, layer1_size);
    fprintf(fvout_txt, "%lld %lld\n", vocab_size, layer1_size);
    for (a = 0; a < vocab_size; a++) {
      fprintf(fo, "%s ", vocab[a].word);
      fprintf(fvout, "%s ", vocab[a].word);
      fprintf(fvout_txt, "%s ", vocab[a].word);
      if (binary) for (b = 0; b < layer1_size; b++) fwrite(&syn0[a * layer1_size + b], sizeof(real), 1, fo);
      else for (b = 0; b < layer1_size; b++) fprintf(fo, "%lf ", syn0[a * layer1_size + b]);
      // if (binary) 
      for (b = 0; b < layer1_size; b++) fwrite(&syn1neg[a * layer1_size + b], sizeof(real), 1, fvout);
      // else
      for (b = 0; b < layer1_size; b++) fprintf(fvout_txt, "%lf ", syn1neg[a * layer1_size + b]);
      fprintf(fo, "\n");
      fprintf(fvout, "\n");
      fprintf(fvout_txt, "\n");
    }
